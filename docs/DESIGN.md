...
Current baseline: base scales live in `src/ar/scene.ts` (`SOLAR_SYSTEM_MODEL_SCALE`, `SOLAR_FALLBACK_SCALE`) with a modest mobile multiplier in `src/main.ts` (`getSolarScaleMultiplier`). The loaded `solar_system.glb` is tidied at runtime by hiding decorative asteroid/dwarf clutter, shrinking the Sun node, and centering the fitted model over the Hiro marker. Detail panel preview uses `PANEL_PREVIEW_TARGET_SIZE` / `PANEL_PREVIEW_LARGE_TARGET_SIZE`, device-pixel-ratio canvas sync, centering after bounding-box fit, and per-planet `previewScale` from `src/data/planets.ts`.
